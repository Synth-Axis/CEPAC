<?php

require_once "../../db/config.php";

// EDIT CS TEXT
if ( isset($_POST['edit-text'])  ) {

    $text = $_POST["text"];
    $content_id = $_POST["content_id"];

    if(!empty($text)) {
                
        foreach( $text as $key => $val ) {
            $sql = $db->prepare("UPDATE content SET text = ? WHERE content_id = ? ");
            $n_text = $val;
            $n_content_id = $content_id[$key];
            
            $sql->execute([
                $n_text,
                $n_content_id
            ]);
        }
        header("Location: ../../backoffice/projects.php");
    }

    header("Location: ../../backoffice/projects.php");
}

// ADD SLIDER IMAGE
if (isset($_POST['add-img'])) {

    $allowed_types = [
        "jpg" => "image/jpeg",
        "png" => "image/png",
        "webp" => "image/webp"
    ];

    $iorder = isset($_POST["iorder"]) ? trim($_POST['iorder']) : NULL;
    $tag = "pj";

    if(
        !empty($iorder) &&
        isset($_FILES["newimg"]) &&
        !empty($_FILES["newimg"]) && 
        $_FILES["newimg"]["size"] > 0 && 
        $_FILES["newimg"]["error"] === 0 &&
        in_array($_FILES["newimg"]["type"], $allowed_types)
    ) {
                   
        $file_extension = array_search($_FILES["newimg"]["type"], $allowed_types);
        $filename = date("YmdHis") ."_". mt_rand(100000, 999999).".".$file_extension;

        $folder = "images/fazemos/".$filename;

        $sql = "INSERT INTO images (src, folder, iorder, tag)  VALUES (?, ?, ?, ?) ";
        
        $query = $db->prepare($sql);

        $query->execute([
            $filename,
            $folder,
            $iorder,
            $tag
        ]);

        move_uploaded_file($_FILES["newimg"]["tmp_name"], "../../images/fazemos/".$filename);

        header("Location: ../../backoffice/projects.php");
    }
    header("Location: ../../backoffice/projects.php");

}

// EDIT SLIDER IMG
if (isset($_POST['edit-img'])) {

    $allowed_types = [
        "jpg" => "image/jpeg",
        "png" => "image/png",
        "webp" => "image/webp"
    ];

    $img_id = $_POST["img_id"];
    $oldimg = $_POST["oldimg"];
    $iorder = isset($_POST["iorder"]) ? trim($_POST['iorder']) : NULL;

    if( isset($iorder)) {

        $sql = "UPDATE images SET iorder = '".$iorder."' WHERE img_id = '".$img_id."' ";
        
        $query = $db->prepare($sql);

        $query->execute();
    }

    if(
        isset($_FILES["newimg"]) &&
        !empty($_FILES["newimg"]) && 
        $_FILES["newimg"]["size"] > 0 && 
        $_FILES["newimg"]["error"] === 0 &&
        in_array($_FILES["newimg"]["type"], $allowed_types)
    ) {
                  
        unlink("../../images/fazemos/".$oldimg);    

        $file_extension = array_search($_FILES["newimg"]["type"], $allowed_types);
        $filename = date("YmdHis") ."_". mt_rand(100000, 999999).".".$file_extension;

        $folder = "images/fazemos/".$filename;

        $sql = "UPDATE images SET src = '".$filename."', folder = '".$folder."' WHERE img_id = '".$img_id."' ";
        
        $query = $db->prepare($sql);

        $query->execute();
        move_uploaded_file($_FILES["newimg"]["tmp_name"], "../../images/fazemos/".$filename);

        header("Location: ../../backoffice/projects.php");
    }
    header("Location: ../../backoffice/projects.php");
}

$type = isset($_POST['type']) ? $_POST['type'] : '';

// DELETE SLIDER IMGS
if($type == 18) {
    $id = $_POST["id"];
    $src =  !empty($_POST["src"]) ? trim($_POST["src"]) : NULL;


    unlink("../../images/fazemos/".$src); 
        
    $query = $db->prepare("DELETE FROM images WHERE img_id = :img_id");

    $query->bindParam( ":img_id",  $param_img_id, PDO::PARAM_INT );

	$param_img_id = $id;

    $query->execute();
        
}

// ADD PROJECT
if (isset($_POST['add-project'])) {


    $porder = isset($_POST["porder"]) ? trim($_POST['porder']) : NULL;
    $text = isset($_POST["text"]) ? trim($_POST['text']) : NULL;
    $title = isset($_POST["title"]) ? trim($_POST['title']) : NULL;

    if(
        !empty($porder) &&
        !empty($text) &&
        !empty($title) 
    ) {

        $sql = "INSERT INTO projects (porder, text, title)  VALUES (?, ?, ?) ";
        
        $query = $db->prepare($sql);

        $query->execute([
            $porder,
            $text,
            $title
        ]);

        header("Location: ../../backoffice/projects.php");
    }

    header("Location: ../../backoffice/projects.php");

}


// EDIT PROJECT
if (isset($_POST['edit-project'])) {

    $project_id = isset($_POST["project_id"]) ? $_POST["project_id"] : '';
    $porder = isset($_POST["porder"]) ? $_POST["porder"] : '';
    $title = isset($_POST["title"]) ? $_POST["title"] : '';
    $text = isset($_POST["text"]) ? $_POST["text"] : '';


    if( !empty($project_id) ) {

        $query = $db->prepare("UPDATE projects SET porder = :porder, title = :title, text = :text WHERE project_id = :project_id");

        $query->bindParam( ":porder",  $param_porder, PDO::PARAM_INT );
        $query->bindParam( ":project_id",  $param_project_id, PDO::PARAM_INT );
        $query->bindParam( ":title",  $param_title, PDO::PARAM_STR );
        $query->bindParam( ":text",  $param_text, PDO::PARAM_STR );

        $param_porder = $porder;
        $param_project_id = $project_id;
        $param_title = $title;
        $param_text = $text;
        
        $query->execute();

        header("Location: ../../backoffice/projects.php");

    }

    header("Location: ../../backoffice/projects.php");
}

// DELETE PROJECT
if($type == 19) {
    $id = $_POST["id"];
        
    $query = $db->prepare("DELETE FROM projects WHERE project_id = :project_id");

    $query->bindParam( ":project_id",  $param_project_id, PDO::PARAM_INT );

	$param_project_id = $id;

    $query->execute();
        
}


// EDIT BANNER
if (isset($_POST['edit-banner'])) {

    $allowed_types = [
        "jpg" => "image/jpeg",
        "png" => "image/png",
        "webp" => "image/webp"
    ];

    $banner_id = $_POST["banner_id"];
    $oldimg = $_POST["oldimg"];
    $title = isset($_POST["title"]) ? trim($_POST['title']) : NULL;

    if( isset($title)) {

        $sql = "UPDATE banners SET title = '".$title."' WHERE banner_id = '".$banner_id."' ";
        
        $query = $db->prepare($sql);

        $query->execute();
    }

    if(
        isset($_FILES["newimg"]) &&
        !empty($_FILES["newimg"]) && 
        $_FILES["newimg"]["size"] > 0 && 
        $_FILES["newimg"]["error"] === 0 &&
        in_array($_FILES["newimg"]["type"], $allowed_types)
    ) {
                  
        unlink("../../images/banners/".$oldimg);    

        $file_extension = array_search($_FILES["newimg"]["type"], $allowed_types);
        $filename = date("YmdHis") ."_". mt_rand(100000, 999999).".".$file_extension;

        $folder = "images/banners/".$filename;

        $sql = "UPDATE banners SET src = '".$filename."', folder = '".$folder."' WHERE banner_id = '".$banner_id."' ";
        
        $query = $db->prepare($sql);

        $query->execute();
        move_uploaded_file($_FILES["newimg"]["tmp_name"], "../../images/banners/".$filename);

        header("Location: ../../backoffice/projects.php");

    }
    
    header("Location: ../../backoffice/projects.php");

}